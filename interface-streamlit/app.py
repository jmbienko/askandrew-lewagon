...
if prompt := st.chat_input('Just ask me'):
    ...
    fastapi_url = "https://laiatoo-tuxqyxiooq-ew.a.run.app/chat"  # Adjust the URL based on your FastAPI server location
    ...
    # Preparing YouTube search query URLs
    youtube_base_url = "https://www.youtube.com/results?search_query="
    links_with_titles = []
    processed_titles = set()  # Set to track which titles have already been added

    for index, source in enumerate(sources, start=1):
        title = source.replace('/Users/larshofferbert/code/hofferBERT/askandrew_langchain/docs/', '').replace('.txt', '')

    # Check if the title has already been processed to avoid duplicates
        if title not in processed_titles:
           processed_titles.add(title)  # Mark this title as processed
           youtube_query = '+'.join(title.split(' ')).replace('&', '%26')
        # Format each link with its index
           markdown_link = f"{index}. [{title}]({youtube_base_url + youtube_query})"
           links_with_titles.append(markdown_link)

# Adjusting the joining to ensure clarity in presentation
    sources_list = '\n'.join(links_with_titles)
    llm_answer = response['response']['result']
    final_answer = f"{llm_answer} \n\n**Find more in episodes:** \n\n{sources_list}"
